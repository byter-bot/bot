    ...
    async def update_status(self):
        if random.randint(0, 25) == 15:
            await self.bot.change_presence(
                activity=random.choice([
                    ...
                    discord.Game(name='did you know? this status only exists <1% of the time'), None
                ])
            )
            return
        ...
        await self.bot.change_presence(
            activity=random.choice([
                ...
                discord
                .Activity(name=f'for {self.bot.config["prefix"][0]}help', type=discord.ActivityType.watching),
                discord.Activity(
                    name=(
                        'discord for ' + str(
                            int((datetime.datetime.now().timestamp() - psutil.Process().create_time()) / 3600)
                        ) + ' hours!'
                    ),
