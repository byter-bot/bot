...
class EmbedHelpCommand(commands.HelpCommand):
    async def send_bot_help(self, mapping: Mapping[Optional[commands.Cog], List[commands.Command]]):
        ...
        for category, cmds in itertools.groupby(filtered, key=get_category):
            ...
            embed.add_field(
                name=category, value=', '.join(map(str, sorted(cmds, key=lambda c: c.name))), inline=False
            )
    ...
    async def send_command_help(self, command: commands.Command):
        ctx: commands.Context = self.context
        embed = discord.Embed(color=0x5050fa, title=command.name, description=command.help or '')
        ...
        embed.add_field(name='Category', value=command.cog.qualified_name if command.cog else 'No category')

        embed.add_field(name='Aliases', value=', '.join(f'`{i}`' for i in command.aliases) or 'None')

        embed.add_field(
            name='Arguments', value=', '.join(f'`{i}`' for i in command.signature.strip().split()) or 'None'
        )
    ...
    async def send_group_help(self, group: commands.Group):
        ctx: commands.Context = self.context
        embed = discord.Embed(color=0x5050fa, title=group.name, description=group.help or '')
        if not group.hidden:
        ...
        embed.add_field(
            name='Subcommands', value=', '.join(f'**{cmd}** - {cmd.short_doc}' for cmd in group.commands)
        )

        embed.add_field(name='Category', value=group.cog.qualified_name if group.cog else 'No category')

        embed.add_field(name='Aliases', value=', '.join(f'`{i}`' for i in group.aliases) or 'None')

        embed.add_field(
            name='Arguments', value=', '.join(f'`{i}`' for i in group.signature.split()) or 'None'
        )
    ...
    async def _commands(self, ctx: commands.Context):
        ...
        await ctx.send(
            embed=discord.Embed(
                ...
                description=', '.join(
                    f'`{cmd}`' for cmd in self.bot.walk_commands() if not cmd.hidden and cmd.can_run(ctx)
                )
            )
        )
